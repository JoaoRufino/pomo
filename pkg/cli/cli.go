...
func NewPomoCli() (*PomoCli, error) {
	...
	pomoCli.client, err = runner.NewClient(pomoCli.Config())
	if err != nil {
		return nil, err
	}
...
func (pomoCli *PomoCli) Server() *pomo.Server {
	return pomoCli.server
}

func (pomoCli *PomoCli) Client() runner.Client {
	return pomoCli.client
}
func (pomoCli *PomoCli) SetServer(server *pomo.Server) {
